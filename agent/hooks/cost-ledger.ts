...
import { reviewRouteState, currentReviewRoute } from "../lib/review-route";
...
function bindScope(ctx: HookContext, turnId: string, stepIndex: number): void {
  ...
  const route = ctx.channel.kind === "subagent" ? reviewRouteState.get() : currentReviewRoute(ctx.channel.kind, []);
  costExecutionScope.update(() => ({
    ...
    phase: route?.role === "lane" ? `lane:${route.axis}` : route?.role === "coordinator" ? route.task === "presentation" ? "presentation" : "coordination" : route?.role ?? "coordination",
  }));
}
