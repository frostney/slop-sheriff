...
export function registerReviewWorkRoutes(http: HttpRouter, isAuthorized: (request: Request) => Promise<boolean>): void {
  http.route({ path: "/review-work/put", method: "POST", handler: httpAction(async (ctx, request) => {
    ...
    const parsed = z.strictObject({ currentAttemptId: z.string().min(1), envelope: completedWorkEnvelopeSchema }).safeParse(await request.json().catch(() => null));
    if (!parsed.success) return new Response(null, { status: 400 });
    const { currentAttemptId, envelope } = parsed.data;
    ...
    const result = await ctx.runMutation(internal.reviewWorkData.put, { currentAttemptId, binding: envelope.binding, signature: envelope.signature, storageId, byteLength: new TextEncoder().encode(envelope.data).byteLength });
